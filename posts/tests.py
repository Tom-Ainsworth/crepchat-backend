...
class PostListViewTests(APITestCase):
    ...
    def test_can_list_posts(self):
        ...
        Post.objects.create(owner=tom, title='title')
...
class PostDetailViewTests(APITestCase):
    def setUp(self):
        ...
        Post.objects.create(owner=tom, title='toms post', content='toms content')
        Post.objects.create(owner=ellie, title='ellies post', content='ellies content')
        
        
    
    def test_user_can_retrieve_post_with_valid_id(self):
        response = self.client.get('/posts/1/')
        self.assertEqual(response.data['title'], 'toms post')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
